...
exports.generateBotXpValues = exports.sampleBots = void 0;
const BOT_POOL = [
    // Indian names
 ...
 */
const sampleBots = (count) => {
    return shuffle(BOT_POOL).slice(0, count);
};
exports.sampleBots = sampleBots;
/**
 ...
 */
const generateBotXpValues = () => {
    const values = [];
    // Top tier (3 bots)
    for (let i = 0; i < 3; i++)
        values.push(randInt(800, 1500));
    // Upper tier (7 bots)
    for (let i = 0; i < 7; i++)
        values.push(randInt(400, 799));
    // Mid tier (10 bots)
    for (let i = 0; i < 10; i++)
        values.push(randInt(100, 399));
    // Low tier (7 bots)
    for (let i = 0; i < 7; i++)
...
exports.generateBotXpValues = generateBotXpValues;
